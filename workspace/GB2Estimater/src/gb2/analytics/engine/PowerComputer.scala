...
class PowerComputer(val league: MongoCollection, val division: String, val dist: Array[Double]) {
  ...
  val matches = league.find(MongoDBObject("Div" -> division, "HomeTeam" -> MongoDBObject("$exists" -> true), "AwayTeam" -> MongoDBObject("$exists" -> true), "FTHG" -> MongoDBObject("$exists" -> true), "FTAG" -> MongoDBObject("$exists" -> true)), MongoDBObject("_id" -> 1, "Div" -> 1, "Date" -> 1, "HomeTeam" -> 1, "AwayTeam" -> 1, "FTHG" -> 1, "FTAG" -> 1)).sort(MongoDBObject("Date" -> 1)).toList
  ...
  matches.foreach { m =>
    {
      val id = m("_id").asInstanceOf[ObjectId].toString()
      FTHGs += (id -> m("FTHG").asInstanceOf[Int])
      FTAGs += (id -> m("FTAG").asInstanceOf[Int])
      HomeTeams += (id -> m("HomeTeam").asInstanceOf[String])
      AwayTeams += (id -> m("AwayTeam").asInstanceOf[String])
    }
  }
      ...
      matchDaysAndResults(d).foreach { m =>
        {
          if (d < currentDay) {
            ...
            if (interval > 0 && interval <= windowDays) {
              ...
              home += FTHGs(m).toDouble
              away += FTAGs(m).toDouble
            }
          }
        }
      }
  ...
  def computePower(
    ...
    teamCategory: String,
    currentDay: DateTime,
    matchAndDays: collection.mutable.Map[String, Map[DateTime, String]],
      ...
      for ((d, m) <- matchAndDays(team)) {
        if (d < currentDay) {
          ...
          if (interval > 0 && interval <= windowDays) {
            ...
            totalWeight += dist(diff)
            if ("H" == teamCategory) {
              goals += FTHGs(m).toDouble
              conceded += FTAGs(m).toDouble
            } else {
              goals += FTAGs(m).toDouble
              conceded += FTHGs(m).toDouble
            }
          }
        }
      }
        ...
        allMatches.foreach { m =>
          {
            ...
            if (m.contains(teamCategory) && m(teamCategory) == team && !teamMatchDayAndResult(team).contains(d)) {
              teamMatchDayAndResult(team)(d) = m("_id").asInstanceOf[ObjectId].toString()
            }
          }
        }
      }
  ...
  def obtainMatchDayAndResult(allMatches: List[DBObject]): collection.mutable.Map[DateTime, ListBuffer[String]] = {
    var matchDayAndResult = collection.mutable.Map[DateTime, ListBuffer[String]]()
    allMatches.foreach { m =>
      {
        val d = new DateTime(m("Date"))
        if (!matchDayAndResult.contains(d)) {
          matchDayAndResult(d) = new ListBuffer[String]()
        }
        matchDayAndResult(d) += m("_id").asInstanceOf[ObjectId].toString()
      }
    }
