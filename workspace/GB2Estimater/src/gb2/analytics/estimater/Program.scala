...
object Program {
  ...
  def processOneDiv(mongoClient: MongoClient, divName: String, windowDays: Int, model: RegressionModel, tag: String) = {
    var leagueName = ""
    val leaguePattern = """(.*)\d+""".r
    divName match {
      case leaguePattern(name) => leagueName = name
    }
    val league = mongoClient("gb2data")(leagueName)
    val div = league.filter { x => x("Div") == divName }
    val dist = model.distribution(windowDays)
    val builder: BulkWriteOperation = league.initializeOrderedBulkOperation

    val powerComputer: PowerComputer = new PowerComputer(league, divName, dist);
    powerComputer.execute()

    val matches = league.find(MongoDBObject("Div" -> divName, "HomeTeam" -> MongoDBObject("$exists" -> true), "AwayTeam" -> MongoDBObject("$exists" -> true), "FTHG" -> MongoDBObject("$exists" -> true), "FTAG" -> MongoDBObject("$exists" -> true))).sort(MongoDBObject("Date" -> 1)).toList
